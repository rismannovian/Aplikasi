...
class myPDF extends FPDF{
	function header(){
		...
		$this->Cell(275,7,'LAPORAN DATA SURAT KELUAR',0,0,'C');		
		$this->Ln(10);
	}
	function footer(){
		$this->SetY(-15);
	...
	function viewtable($db){
		...
		$stmt= $db->query("SELECT * from surat_keluar where no_agenda='$no_agenda'");
		 while ($data = $stmt->fetch(PDO::FETCH_OBJ)) {
			...
			$this->Cell(35,10,$data->perihal,1,0,'C');
			$this->Ln();
		}
